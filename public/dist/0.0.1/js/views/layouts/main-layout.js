...
// src/client/views/layouts/main-layout.js
var require_main_layout = __commonJS({
  "src/client/views/layouts/main-layout.js"(exports, module) {
    var { BaseLayout } = require_base_layout();
    var { ScrollActionTo, ScrollActionTop, ScrollEventType, SCROLL_THRESHOLD } = require_scroll();
    var { changeLangPath, toggleQueryParameter } = require_utils2();
    var { navigateHandler } = require_utils();
    var { context } = require_globals2();
    var { DEFAULT_LANGUAGE, LANGUAGES } = require_globals();
    var _MainLayout = class extends BaseLayout {
      ...
      async init(parent, firstTime) {
        ...
        if (this.node) {
          ...
          this.navIcon = this.appBarElem?.querySelector('[data-button="navigation"]');
          this.searchIcon = this.appBarElem?.querySelector('[data-button="search"]');
          this.drawerElem = this.node.querySelector(".drawer");
          this.drawerElem?.classList.remove("drawer-hoverable");
          this.headerIconBtn = this.drawerElem?.querySelector('[data-button="header-navigation"]');
          ...
          this.signInUpElem = drawerAccountBar?.querySelector('[data-content="sign-in-up"]');
          this.signOutElem = drawerAccountBar?.querySelector('[data-content="sign-out"]');
          const drawerLangBar = this.drawerElem?.querySelector(".drawer-lang-bar");
          this.langElem = drawerLangBar?.querySelector('[data-content="lang"]');
          this.langImageElem = drawerLangBar?.querySelector('[data-image="lang"]');
          this.langList = drawerLangBar?.querySelector('[data-list="lang"]');
          this.list = this.drawerElem?.querySelector('[data-list="main"]');
          this.searchPanel = this.appBarElem?.querySelector(".search");
          this.searchForm = this.searchPanel?.querySelector("form");
...
export default require_main_layout();
